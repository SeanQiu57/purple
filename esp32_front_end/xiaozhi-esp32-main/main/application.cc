...
#include <arpa/inet.h>
#if CONFIG_BOARD_ZHENGCHEN_1_54TFT_WIFI
#include "boards/zhengchen-1.54tft-wifi/zhengchen_lcd_display.h"
#endif

#define TAG "Application"
#ifndef FONT_AWESOME_MICROPHONE
...
void Application::ForceStopListening()
{
    if (device_state_ == kDeviceStateListening) // 只在真正 Listening 时动作
    {
        protocol_->SendStopListening(); // 通知服务器
        audio_decode_queue_.clear();    // 清理尚未播放的 TTS
        audio_decode_cv_.notify_all();
        SetDeviceState(kDeviceStateIdle); // 回到 Idle
    }
}
...
void Application::ToggleChatState()
{
    ...
    if (device_state_ == kDeviceStateIdle)
    {
        Schedule([this]()
            ...
            SetListeningMode(realtime_chat_enabled_ ? kListeningModeManualStop : kListeningModeAutoStop); });
    }
...
void Application::TestWheelPins()
{
    // 配置成输入，上拉（假设按下接地）
    gpio_config_t cfg = {};
    cfg.pin_bit_mask = (1ULL << WHEEL_UP_GPIO) | (1ULL << WHEEL_DOWN_GPIO);
    ...
    while (true)
    {
        int up = gpio_get_level(WHEEL_UP_GPIO);
        int down = gpio_get_level(WHEEL_DOWN_GPIO);
        ESP_LOGI(TAG, "Wheel → Up=%d  Down=%d", up, down);
...
void Application::SetListeningMode(ListeningMode mode)
{
    ...
    listening_mode_ = mode; // 保存新模式

    /* ---------- 2. Manual / Idle → Auto 时：立即开始聆听 ---------- */
    if (mode == kListeningModeAutoStop &&
        device_state_ == kDeviceStateIdle)
    {
        Schedule([this]() // 用 Schedule 避免在同一调用栈里阻塞
                 {
                     if (!protocol_->IsAudioChannelOpened())
                     { // 先连通道
                         SetDeviceState(kDeviceStateConnecting);
                     ...
                     SetDeviceState(kDeviceStateListening); // 真正进入聆听
                 });
    }
...
void Application::UpdateListeningModeUI()
{
    auto *display = Board::GetInstance().GetDisplay();
    const char *txt =
        (listening_mode_ == kListeningModeAutoStop) ? "自动" : "手动";
    display->SetListeningModeIndicator(txt); // 通用接口
}
