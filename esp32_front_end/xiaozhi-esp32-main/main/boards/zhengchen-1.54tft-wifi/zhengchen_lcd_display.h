...
protected:
    ...
    lv_obj_t *mode_label_ = nullptr;      // 模式图标标签
...
public:
    ...
    void InitCustomUI()
    {
        SpiLcdDisplay::SetupUI(); // 调用基类受保护函数
        SetupHighTempWarningPopup();

        // mode_label_ = lv_label_create(lv_scr_act());
        // lv_obj_align(mode_label_, LV_ALIGN_BOTTOM_MID, 0, -2);
        // lv_label_set_text(mode_label_, "手动");
    }

    void UpdateModeText(ListeningMode mode)
    {
        if (!mode_label_)
            return;
        const char *txt = (mode == kListeningModeAutoStop) ? "自动" : "手动";
        lv_label_set_text(mode_label_, txt);
    }
