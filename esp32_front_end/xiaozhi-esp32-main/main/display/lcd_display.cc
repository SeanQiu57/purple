...
void LcdDisplay::SetupUI()
{
    ...
    listening_mode_label_ = lv_label_create(status_bar_);
    lv_obj_set_style_text_font(listening_mode_label_, fonts_.icon_font, 0);
    lv_obj_set_style_text_color(listening_mode_label_, current_theme.text, 0);
    lv_label_set_text(listening_mode_label_, ""); // 默认空
    lv_obj_set_style_margin_left(listening_mode_label_, 5, 0);
}
...
void LcdDisplay::SetupUI()
{
    ...
    lv_obj_center(low_battery_label_);
    lv_obj_add_flag(low_battery_popup_, LV_OBJ_FLAG_HIDDEN);
    listening_mode_label_ = lv_label_create(lv_scr_act());
    // 这里用 text_font，否则中文也出方框
    lv_obj_set_style_text_font(listening_mode_label_, fonts_.text_font, 0);
    lv_obj_set_style_text_color(listening_mode_label_, current_theme.text, 0);
    lv_label_set_text(listening_mode_label_, ""); // 初始空
    lv_obj_align(listening_mode_label_, LV_ALIGN_BOTTOM_MID, 0, -4);
}
#endif
...
void LcdDisplay::SetListeningModeIndicator(const char *text)
{
    DisplayLockGuard lock(this);
    if (!listening_mode_label_)
        return;

    // 用主文本字体，而不是 icon_font
    lv_obj_set_style_text_font(listening_mode_label_, fonts_.text_font, 0);
    lv_label_set_text(listening_mode_label_, text);
}
